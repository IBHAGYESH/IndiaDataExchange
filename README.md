...
A decentralized data marketplace built on Algorand Testnet where sellers upload Indian datasets and buyers purchase them using USDC via the x402 payment protocol. AI agents can autonomously browse and purchase datasets using HTTP 402 payments.
...
└── backend/           # Express + TypeScript + MongoDB + Pinata IPFS
    ...
    └── AlgoKit/       # Algorand smart contracts (Puya)
        └── smart_contracts/
            ├── AlgoStakeX/          # Reference staking contract
...
const data = await payingFetch("http://api.indiadataexchange.xyz/api/datasets/abc/download");
```
...
- All wallet operations use Pera Wallet (testnet)
